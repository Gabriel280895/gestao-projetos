...
with st.sidebar:
    ...
    st.markdown(
        ...
        <div style="text-align: center; color: rgba(255,255,255,0.7); font-size: 13px; margin-top: 20px;">
            ...
            <p style="font-size: 11px; color: rgba(255,255,255,0.5);">v1.8 - Galeria Edition</p>
        </div>
        """, 
...
# =========================================================
# 4. TAREFAS (COM CORES E ÍCONES RESTAURADOS)
# =========================================================
elif menu == "Tarefas":
    ...
    if not opts:
        ...
        with t_tab1:
            ...
            with c2:
                ...
                for _, t in tv[tv['status'] == "Fazendo"].iterrows():
                    # Usa st.warning para dar a cor de fundo amarela
                    st.warning(f"**{t['title']}**\n\n👤 {t['owner']}", icon="🏗️")
                    # Botões de ação rápida fora do form para agilidade
                    with st.expander("⚙️ Gerenciar"):
                         with st.form(f"f2_{t['id']}"):
                            ns = st.selectbox("Mover:", ["A fazer", "Fazendo", "Bloqueado", "Feito"], index=1)
                            np = st.slider("Progresso", 0, 100, int(t['progress']))
                            if st.form_submit_button("Atualizar"):
                                db.execute_command("UPDATE tasks SET status=?, progress=? WHERE id=?", (ns, np, t['id']))
                                st.rerun()

            # --- COLUNA: BLOQUEADO (VERMELHO) ---
            with c3:
                st.markdown("### 🚫 Bloqueado")
                st.markdown("---")
                for _, t in tv[tv['status'] == "Bloqueado"].iterrows():
                    # Usa st.error para dar a cor de fundo vermelha
                    st.error(f"**{t['title']}**\n\n🛑 Travado", icon="🚨")
                    with st.expander("🔓 Resolver"):
                         with st.form(f"f3_{t['id']}"):
                            ns = st.selectbox("Status", ["A fazer", "Fazendo", "Bloqueado", "Feito"], index=2)
            ...
            with c4:
                ...
                for _, t in tv[tv['status'] == "Feito"].iterrows():
                    # Usa st.success para dar a cor de fundo verde
        ...
        with t_tab2:
            with st.form("new_t_form"):
                ...
                ow = st.text_input("Dono (Opcional)")
